...
export COLOR1="$(cat /etc/ALVI/theme/$colornow | grep -w "TEXT" | cut -d: -f2|sed 's/ //g')"
export COLBG1="$(cat /etc/ALVI/theme/$colornow | grep -w "BG" | cut -d: -f2|sed 's/ //g')"
WH='\033[1;37m'
###########- END COLOR CODE -##########
...
#read -rp "Enter Token (Contact ALVI CELL) : " -e token
...
LLatest=`date`
Get_Data () {
git clone https://github.com/ALVICELL/userbackup.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    cd /root/user-backup
    git config --global user.email "adigemether@gmail.com" &> /dev/null
    git config --global user.name "ALVICELL" &> /dev/null
    rm -rf .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
    git commit -m $NameUser &> /dev/null
    git branch -M main &> /dev/null
    git remote add origin https://github.com/ALVICELL/userbackup.git
    git push -f https://github.com/ALVICELL/userbackup.git &> /dev/null
}
...
echo -e""
echo -e "$yellow Username :👉  ALVI  👈$NC"
echo -e""
echo -e "$yellow PASTEKAN Username GITHUB DI BAWAH INI$NC"
echo -e ""
Save_And_Exit
fi
link="https://raw.githubusercontent.com/ALVICELL/userbackup/main/$NameUser/$NameUser.zip"
sleep 1
...
cekdata=$(curl -sS https://raw.githubusercontent.com/ALVICELL/userbackup/main/$NameUser/$NameUser.zip | grep 404 | awk '{print $1}' | cut -d: -f1)
...
mkdir -p /root/backup
wget -q -O /root/backup/backup.zip "https://raw.githubusercontent.com/ALVICELL/userbackup/main/$NameUser/$NameUser.zip" &> /dev/null
...
echo -e "$COLOR1 ${NC}                ${WH}• ALVI CELL •${NC}                 $COLOR1 $NC"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo -e ""
